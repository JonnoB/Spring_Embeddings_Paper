...
##Experimental stuff
I can't really remember what I was doing here but I am leaving it here for a while until I delete it.
It takes a while to load the data so It is commented out
```{r}
...
# 
# test_out %>%
#   ggplot(aes(x = attack_round_mean, y = blackout_size_mean)) + geom_point()
# 
...
metric_performance_plot_df  <- metric_performance_all   %>%
  ...
  ungroup %>% mutate(error_type ="standard")


metric_performance_plot_df %>%
  group_by(eval_metric, metric, average_type, graph, combo, graph_order) %>%
  summarise(.estimate = mean(.estimate)) %>%
  group_by(eval_metric, graph) %>%
  mutate(rank = ifelse(eval_metric =="rsq", rank(-.estimate), rank(.estimate)),
         graph_order  = factor(gsub("_igraph", "", graph), 
...
error_df  %>%
  ...
  ggplot(aes(x = graph_order, y = .pl_estimate, fill = combo)) + geom_boxplot() +
...
tukey_test <-TukeyHSD(aov_res, which = "metric")
tukey_test
