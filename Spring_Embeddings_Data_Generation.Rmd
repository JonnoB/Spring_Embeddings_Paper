...
parameter_df <- parameter_df %>% mutate(deletion_seed = simulation_id, #The random seed is the simulation id, this means that each simulation id will have the same attack order. This is becuase we are looking at the effect of alpha on collapse not at the effect of deletion order
  ...
  mutate(
    ...
    collapse_path = file.path(collapse_sets_path,sub_path),
    collapse_summary_path = file.path(collapse_set_summaries_path, sub_path),
    embeddings_path = file.path(embeddings_path, permutation, paste0("fract_", fract, "_ec_", ec, "_v_", v, ".rds") )
    )%>%
  ungroup %>%
...
parameter_df <-  list.files(edge_scramble_keys_path, full.names = T, pattern = ".rds") %>% 
  map_df(~{
    ...
    return(temp2)
  }) %>% mutate(deletion_seed = simulation_id, #The random seed is the simulation id, this means that each simulation id will have the same attack order. This is becuase we are looking at the effect of alpha on collapse not at the effect of deletion order
                scramble_network = ifelse(fract==0, FALSE, TRUE)) 


parameter_df <- parameter_df %>%
  left_join(
    tibble(graph_path = list.files(basename(power_grid_graphs_path), full.names = T, pattern = ".rds"), 
  ...
  group_by(simulation_id, ec, permutation) %>% #break the parameter table into sets of twelve where each set contains every ec level. This allows for a longer and more stable clock time
  mutate(compute_group = 1:n()) %>%
  ungroup
...
parameter_df <-  list.files(edge_scramble_keys_path, full.names = T, pattern = ".rds") %>% 
  map_df(~{
    target_order <-.x
    #expand grid creates a two column tibble which is not what I want, so I have to do this two stage process
    temp <- non_scramble_df %>% as.data.frame() %>% # the non-scrambles have a different form so have to be done seperately
      expand_grid(df = ., simulation_id = 1:100)  #
...
#parameter_df_simple <- parameter_df
...
Permuted_IEEE_118_results <- list.files(path = collapse_set_summaries_path, 
