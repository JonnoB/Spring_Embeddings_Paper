...
for job_params in ["IEEE_14", "IEEE_30", "IEEE_57", "IEEE_118", "IEEE_300", "UK_high_voltage"]:

    # Defines the grid that will be loaded and also the indices that will be embedded
    # job_params = ["IEEE_14", "IEEE_30", "IEEE_57", "IEEE_118", "IEEE_300", "UK_high_voltage"]

    grid_name = job_params
    print(grid_name)
    # Get the working directory and store it so I can save the embedding there
    project_folder = "/home/jonno/Dropbox/IEEE_Networks/embeddings/node2vec_pl"

    # The folder the data is stored in
    grid_folder = "/home/jonno/Dropbox/IEEE_Networks/power_grid_graphs/pl_graphml" + \
                  "/" + grid_name + "_graphml"

    # save path of the embedded data
    save_path = project_folder + "/" + grid_name + ".csv"

    files_to_embed = os.listdir(grid_folder)
