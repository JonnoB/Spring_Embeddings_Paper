...
target_path <- "/home/jonno/Dropbox/IEEE_Networks/Permuted_IEEE_118_collapse_set/Permutation_2"

all_dirs <-list.dirs(target_path)[-1] #remove base dir
...
completion_time <- all_dirs %>% map_df(~{
  ...
  pull(V4) %>% as.integer()/100
  
  all_files %>% file.info(.) %>% as_tibble() %>%
    mutate(file_path = all_files) %>%
  arrange(ctime) %>%
  mutate(lag = lag(ctime),
    mins = difftime(ctime, lag, units = "mins" ),
    hours = difftime(ctime, lag, units = "hours" ),
    alpha = alpha) %>% select(alpha, mins, size, hours)
...
worst_case <- completion_time %>%
  group_by(alpha) %>%
  summarise(max = max(as.numeric(mins)),
            min = min(as.numeric(mins)))
...
single_collapse_set <- 48000/12
single_collapse_set
total_compute_time  <-30*48000/12
total_compute_time

total_compute_time/36

completion_time %>% 
  mutate(alpha = factor(alpha)) %>%
  ggplot(aes(x = alpha, y = mins, fill = alpha)) + geom_boxplot() +
...
completion_time %>% 
  mutate(alpha = factor(alpha)) %>%
  ggplot(aes(x = alpha, y = hours*100, fill = alpha)) + geom_boxplot() +
  labs(title = "time to completion by alpha there are 4 repititions of each alpha",
       y = "minutes")
...
completion_time %>%
  ...
  mutate(hours_set = hours*100*4/1000, 
         hours_all_sets = hours_set*30,
         hours_all_sets_all_networks = hours_all_sets*5)

```


#Support code for Myriad script
```{r}

target_order_permutations <- list.files("/home/jonno/Dropbox/IEEE_Networks/target_orders/Permuted_IEEE_118", full.names = T)
...
parameter_df <- parameter_df %>% mutate(deletion_seed = 1:n(), #set the random deletion seed to be the job number. This guarentees a different order for every simulation
  ...
  left_join(tibble(graph_path = list.files(IEEE_permuted_path, full.names = T), 
