...
Generate the target orders in parallel... becuase ain't nobidy got time for that
```{r}
# 
# registerDoParallel(cores=detectCores()) #RUNS ON ALL CORES!
...
1:30 %>%
  walk(~{ #Outer loop choose the permutation
    ...
    #Inner Loop: prepare network according to target-order and then attack. Loop is inside function
    #Scrambled_Edge_SaveMultiAttacks(current_graph, target_orders, DeleteOrders_Edges , Target = "Edges")
    #exectutes the targets in parallel. does not do each target in parallel.
    Scrambled_Edge_SaveMultiAttacks_parallel(current_graph, target_orders, DeleteOrders_Edges , Target = "Edges", cores = detectCores())
  })
...
if(!file.exists(file.path(Project_folder,"Permuted_IEEE_118_strain_set"))){
  dir.create(file.path(Project_folder,"Permuted_IEEE_118_strain_set"))
}


1:length(Permuted_IEEE_118_list) %>% walk(~{
 ...
 plan(multiprocess, workers = 4)
  #plan(sequential)
  future_lapply(X = 1:nrow(target_orders2), 
                FUN = function(X){ #use future lapply to parallelize the process
    ...
    if(file.exists(target$file_path)){
      ...
      final_z <- Standardisesd_solution_finder(Scrambled_edge_cap %>%
                                                 set.edge.attribute(., "Link.Limit", value = temp$Link.Limit),
                                               common_time, common_Iter/1000, common_tol, common_mass)
...
target_orders2 <- 1:length(Permuted_IEEE_118_list) %>% map_df(~{
...
foreach(X = 1:nrow(target_orders2), 
        .packages = c("PowerGridNetworking", "rlang", "dplyr", "igraph", "stringr", "purrr", "tidyr")) %dopar% { #use future lapply to parallelize the process
...
Summary_folder <- file.path(Project_folder, "Summary_Permuted_IEEE_118_collapse_set",
                                paste0("Permutation_",  1)) ###########change this! to make multiple
...
ExtractAttackStats_parallel(RootFolder = "/home/jonno/Dropbox/IEEE_Networks/IEEE118_edges", 
        ...
        ExtractAttackStats(RootFolder = "/home/jonno/Dropbox/IEEE_Networks/Permuted_IEEE_118_collapse_set/Permutation_1", 
                           file.path(Summary_folder),
                           Generation = "Net_Generation",
                           EdgeName = "Link")
...
```{r}
    ...
    #Inner Loop: prepare network according to target-order and then attack. Loop is inside function
    Scrambled_Edge_SaveMultiAttacks_parallel(current_graph, target_orders, DeleteOrders_Edges , Target = "Edges", cores = 8)
```
