...
test_out <- left_join(collapse_point, test_embeddings ) %>%
  select(attack_round, line_load_mean = loading, alpha_a = alpha, strain_mean, v, ec, graph, fract) %>%
  filter(complete.cases(.)) %>%
...
test_out  %>%
  #filter(graph == 57) %>%
    mutate(alpha = 1/alpha) %>%
  pivot_longer(., cols =c("alpha", "strain_mean"), names_to = "metric" ) %>%
...
loess_strain <-  loess(formula = attack_round~ strain_mean, 
      data = temp)
...
Out <- bind_rows(
  ...
  metrics(temp, attack_round, line_load_mean) %>% mutate(type = "alpha",
                                                             class = "raw"),
  metrics(temp, attack_round, strain_mean) %>% mutate(type = "strain",
                                                          class = "raw"),
  #the predictive model
  metrics(model_comp, attack_round, alpha_preds) %>% mutate(type = "alpha",
                                                            class = "loess"),
  metrics(model_comp, attack_round, strain_preds) %>% mutate(type = "strain",
                                                             class = "loess")
  ) %>%
    mutate(graph = .x) %>%
