...
This allows me to set the time for the wallkclock of the HPC

IT looks like I can set the clock to 7 mins for graphs 14-57

20 mins for 118
and 25 for 300

if I do a full 1 fract 1 v aka 100*12 sims then there are 1240 groups, if I add on 0.5 hours for safety then it is 2.4 hours per group. making a total of 25680 hours which is about 3000 hours less than if I do them in blocks of 12 but take the worst case
...
HPC_params <- read_delim(file.path(HPC_startup_parameter_file_path, "test_attack_base_IEEE_118_igraph.txt" ), delim = " ")

# mclapply(1:nrow(HPC_params), mc.cores= 6, FUN = function(i){
#   
#     temp <- HPC_params[i,]
#   
#   compute_group <- temp$compute_group
#   
...
list.files(analysis_parameter_file_path, pattern = "PL") %>%
...
#If this is put in a loop or walk mapping then it will "arguably" calculate the data for all the maps and systems
#Load the params file. 
HPC_params <- read_delim(file.path(HPC_startup_parameter_file_path, "strain_base_IEEE_118_igraph.txt" ), delim = " ")
...
collapse_data <- list.files(file.path(collapse_set_summaries_path, "Permutation_IEEE_118_igraph"), 
...
test<- collapse_data %>%
  filter(fract==1) %>%
  group_by(ec,v, attack_round, ) %>%
  select(nodes, edges , generation, contains("loading"), contains("alpha")) %>%
  summarise_all(mean) 

collapse_point %>%
#  filter(fract ==1) %>%
  ggplot(aes(x = loading, y = attack_round, colour = as.factor(ec))) + geom_point()

#Read the embeddings files
embeddings_data <- list.files(file.path(embeddings_path, "IEEE_14_igraph"), 
                              full.names = TRUE, recursive = TRUE) %>%
  map_df(~{

    specs <- basename(.x) %>% gsub(".rds", "", .) %>%str_split(., "_", simplify = TRUE) %>% t(.) %>% as_tibble(., name_repair = "unique") %>%
      pull(V1)
...
#This is a pure cloud when IEEE-14 is used. Is this correct?
left_join(collapse_point, embeddings_data ) %>%
  select(attack_round, line_load_mean, strain_mean, v, ec) %>%
  filter(complete.cases(.)) %>%
  gather(key = metric, value = value, -attack_round, -v,-ec) %>%
  group_by(metric) %>%
  mutate(value = value/max(value)) %>% #rough way of getting the limits
  ggplot(aes(x = value, y = attack_round, colour = as.factor(ec))) +
  geom_point() +
  facet_grid(~metric)
