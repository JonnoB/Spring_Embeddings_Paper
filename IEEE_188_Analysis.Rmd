...
alpha_vector %>% walk(~{
  ...
  write_rds(final_z, file.path(Project_folder,"Solved_height_networks_PL", paste0("IEEE_118_alpha_", alpha, ".rds")))
})
...
fract_vect %>%
  walk(~{ #begin outer loop
    
    current_fract <- .x
    
    for(n in Scramble_ec_values[cpu_sims]){ #begin for loop
      print(n)
      target_orders2 <- target_orders %>% 
        filter(ec == n, scramble_fract == current_fract)
      
      alpha_folder <-  paste0("Solved_height_networks_alpha_", n*100) %>%
        file.path(Project_folder, paste0("constant_ec_from_alpha_fract_", current_fract), .)
      #create folder if it doesn't already exist
      if(!file.exists(alpha_folder)){
        dir.create(alpha_folder, recursive = TRUE)
      }
      ...
      1:nrow(target_orders2) %>% walk(~{
        ...
        if(file.exists(Iteration_path)){
          ...
          temp <-Create_scrambled_edges(Scrambled_edge_cap, target_orders2$seed[.x], fract = current_fract)
          ...
          write_rds(final_z, file.path( alpha_folder , paste0("IEEE_118_alpha_", .x, ".rds")))
        }
      })
