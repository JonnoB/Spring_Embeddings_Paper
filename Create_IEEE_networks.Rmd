...
5 levels of generator fraction is used either 17, 12, 8, 4, 1, or a single generator 
...
```{r}
 ...
 test <- as_data_frame(exp_IEEE, what = "vertices")
  

gen_id <- test %>% filter(Generation_MW >0) %>% pull(name) 
 #randomly generate x samples 20 times
...
active_gen_df <- map2_df(.x  = combs$x, .y = combs$y, ~{
  ...
  tibble(active_gen_id =  sample(gen_id, .x, replace = FALSE), sample = .y, level = .x)

  }) %>%
  bind_rows(tibble(active_gen_id =  gen_id, sample = 1, level = 17)) %>%
  bind_rows(tibble(active_gen_id =  gen_id, sample = 1:17, level = 1))
 
#trn off all gens>0 that are not in the list
active_gen_id <-active_gen_df %>%
  filter(sample == 1, level == 4) %>%#BOTH OF THESE NEED TO BE .X .Y OR SUFFERING!
pull(active_gen_id)

test <- as_data_frame(exp_IEEE, what = "vertices") %>%
  mutate(Generation_MW = case_when(
    Generation_MW<0 ~Generation_MW,
    name %in% active_gen_id ~ Generation_MW,
    TRUE ~0
  ))


#proportionally balance the network over the remaining generators
