...
if(!file.exists(file.path(Project_folder, "target_orders.rds"))){
  ...
  target_orders <- c(1.05, 1.1, 1.2, 1.5, 2, 3, 5, 7, 10) %>% map_df(~{
...
##Node attack
```{r}
c(1.05, 1.1, 1.2, 1.5, 2, 3, 5, 7, 10) %>% walk(~{
...
for(n in c(1.05, 1.1, 1.2, 1.5, 2, 3, 5, 7, 10)){
  print(n)
...
```{r}
  ...
  target_orders2 <- target_orders %>% 
    filter(ec == 10) #set the ec level

ID <- 6 #for v = 6 which has large deviations at the moment
 print(paste("alpha value", ID))
    #create network
    Scrambled_edge_cap <-  Proportional_Load(IEEE_118, alpha = 10)
    
    set.seed(target_orders2$seed[ID])
    ec_order <- sample(1:ecount(Scrambled_edge_cap), ecount(Scrambled_edge_cap) )
    ...
    OriginBlock_ec_10_6 <- Find_network_balance(List_of_BiConComps[[11]], force = "Net_Generation", 
                                      tstep = common_time, tol = common_tol, distance = "distance", 
                                      maxIter = common_Iter, mass = common_mass)
    
    OriginBlock_ec_10_6_2 <- Find_network_balance(List_of_BiConComps[[11]], force = "Net_Generation", 
                                      tstep = common_time, tol = common_tol, distance = "distance", 
                                      maxIter = common_Iter, mass = common_mass)
    
    
    ec_10_6 <- Create_stabilised_blocks(current_graph, OriginBlock_ec_10_6, 11, force = "Net_Generation", 
                                      tstep = common_time, tol = common_tol, distance = "distance", 
                                      maxIter = common_Iter, mass = common_mass)
    

  params  <- tibble(tstep = c(0.03, 0.03, 0.01, 0.01), Iterations = c(16000, 16000*3, 16000*3, 16000*9))
    
  param_check <- 1:nrow(params) %>%
    map_df(~{
      
          OriginBlock <- Find_network_balance(List_of_BiConComps[[11]], force = "Net_Generation",
                                        tstep = common_time, tol = common_tol, distance = "distance", 
                                        maxIter = common_Iter, mass = common_mass, 
                                        window_size = round(common_Iter/10))$NodeList %>%
            mutate(tstep = params$tstep[.x],
                   maxIter = params$Iterations[.x])
      return(OriginBlock)
    })
  
  saveRDS(param_check, file.path(Project_folder, "Param_check.rds"))
  
  #Infinity topological loading
param_Inf <-  "/media/jonno/Seagate Expansion Drive/IEEE_Networks/Solved_height_networks/IEEE_118_alpha_Inf.rds" 

  
  check_node_height <- param_check %>%
        mutate(type = paste("type",tstep, maxIter, sep = "_")) %>%
    select(node, z, type) %>%
    spread(key = type, value = z) %>%
    mutate(diff = type_0.01_144000-type_0.03_16000,
           abs_diff = abs(diff))
...
ID <- 6 #for v = 6 which has large deviations at the moment
 print(paste("alpha value", ID))
    #create network
    Scrambled_edge_cap <-  Proportional_Load(IEEE_118, alpha = 10000)
   ...
   # set.seed(target_orders2$seed[ID])
    ...
    temp <- as_data_frame(Scrambled_edge_cap) %>%
      mutate(ec = Link.Limit-abs(PowerFlow), #get excess capacity
             ec2 = ec, #order of excess capacity not changed
             alpha = (abs(PowerFlow)+ec2)/abs(PowerFlow), #calculate alpha
             Link.Limit = abs(PowerFlow)+ec2)#re-order link limit
    ...
    List_of_BiConComps <- Create_balanced_blocks(current_graph, force = "Net_Generation")
      ...
      OriginBlock_10k <- Find_network_balance(List_of_BiConComps[[11]], force = "Net_Generation", 
                                        tstep = common_time, tol = common_tol, distance = "distance", 
                                        maxIter = common_Iter, mass = common_mass, 
                                        window_size = round(common_Iter/10))
  
  alpha_10k <- Create_stabilised_blocks(current_graph, OriginBlock_10k, 11, force = "Net_Generation", 
                                        tstep = common_time, tol = common_tol, distance = "distance", 
                                        maxIter = common_Iter, mass = common_mass, 
                                        window_size = round(common_Iter/10))
```
...
ID <- 6 #for v = 6 which has large deviations at the moment
 print(paste("alpha value", ID))
    #create network
    Scrambled_edge_cap <-  Proportional_Load(IEEE_118, alpha = Inf)
    ...
    List_of_BiConComps <- Create_balanced_blocks(current_graph, force = "Net_Generation")
      ...
      OriginBlock_Inf <- Find_network_balance(List_of_BiConComps[[11]], force = "Net_Generation", 
                                        tstep = common_time, tol = common_tol, distance = "distance", 
                                        maxIter = common_Iter, mass = common_mass, 
                                        window_size = round(common_Iter/10))
  
  alpha_Inf <- Create_stabilised_blocks(current_graph, OriginBlock_Inf, 11, force = "Net_Generation", 
                                        tstep = common_time, tol = common_tol, distance = "distance", 
                                        maxIter = common_Iter, mass = common_mass, 
                                        window_size = round(common_Iter/10))
  
  OriginBlock_Inf2 <- Find_network_balance(List_of_BiConComps[[11]], force = "Net_Generation", 
                                        tstep = common_time, tol = common_tol, distance = "distance", 
                                        maxIter = common_Iter, mass = common_mass, 
                                        window_size = round(common_Iter/10))
  ...
  param_check <- 1:nrow(params) %>%
   map_df(~{
          ...
          OriginBlock <- Find_network_balance(List_of_BiConComps[[11]], force = "Net_Generation",
                                        tstep = common_time, tol = common_tol, distance = "distance", 
                                        maxIter = common_Iter, mass = common_mass)$NodeList %>%
            mutate(tstep = params$tstep[1],
                   maxIter = params$Iterations[1])
      return(OriginBlock)
    })
...
Even after doing a 0.01 timestep and over 140k reps the angle is still way off
I don't understand what is happening

```{r}
alpha_Inf_15k <- alpha_Inf %>%
    Calc_weighted_angle(IEEE_118, .)  

 ec_10_6_15k <- ec_10_6 %>%
   Calc_weighted_angle(IEEE_118, .)  
 
 alpha_10_15k <- alpha_10k %>%
    Calc_weighted_angle(IEEE_118, .) 
 
 test_stop <- OriginBlock_Inf2$results %>%
   mutate(
          window_sd = rollapply(theta_rads, width =5000, FUN = sd, fill = NA, align = "right"),
          perc = window_sd/theta_rads)
 
 test_stop %>%
 #  filter(t>150, t<200) %>%
   
 ggplot(aes(x = t, y = perc)) + geom_line()
 +
   coord_cartesian(ylim = c(0,1))
 
 
test <- bind_rows(
 #OriginBlock_10k$results %>% mutate(type = "10k"),
 OriginBlock_ec_10_6$results %>% mutate(type = "ec"),
 OriginBlock_ec_10_6_2$results %>% mutate(type = "ec 2"),
 OriginBlock_Inf$results %>% mutate(type = "Inf"),
 OriginBlock_Inf2$results %>% mutate(type = "Inf 2")
 )
 

test %>%
  ggplot(aes(x = t, y = theta_degs, colour = type)) + geom_line() 
 
 #Only the large component is smaller in the ec_network. This makes no sense.
 left_join(alpha_Inf_15k %>% select(block, alpha_Inf = theta_degs), 
           alpha_Inf_15k %>% select(block, alpha_10 = theta_degs)) %>%
   left_join(., 
             ec_10_6_15k %>% select(block, ec_10 = theta_degs)) %>%
   mutate(alpha_10_small = alpha_10<alpha_Inf,
 ec_10_small = ec_10<alpha_Inf)
 

check_Inf_height <-  OriginBlock %>%
        mutate(type = paste("type",tstep, maxIter, sep = "_")) %>%
    select(node, z, type) %>%
    spread(key = type, value = z) %>%
    mutate(diff = type_0.01_144000-type_0.03_16000,
...
check_Inf_height <- OriginBlock %>%
  left_join(., read_rds("/media/jonno/Seagate Expansion Drive/IEEE_Networks/Solved_height_networks/IEEE_118_alpha_Inf.rds" ), by = "node") %>%
  mutate(diff = z.x-z.y,
         abs_diff = abs(diff))
...
comp <- OriginBlock %>%
  filter(tstep == 0.01) %>%
  select(node, z) %>%
  left_join(check_node_height) %>%
  mutate( diff2 = type_0.01_144000-z)
