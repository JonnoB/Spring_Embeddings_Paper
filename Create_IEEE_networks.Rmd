...
alpha_vector %>% walk(~{
  ...
  final_z2 <- Create_stabilised_blocks(current_graph, OriginBlock, 11, force = "Net_Generation", 
                                      tstep = common_time, tol = common_tol, distance = "distance", 
                                      maxIter = common_Iter, mass = common_mass)
 ...
 print(paste("alpha value", ID))
  ...
  param_check <- 1:nrow(params) %>%
    map_df(~{
          ...
          OriginBlock <- Find_network_balance(List_of_BiConComps[[11]], force = "Net_Generation",
                                        tstep = params$tstep[.x], tol = 1e-10, distance = "distance",
                                        maxIter = params$Iterations[.x], mass = 5000)$NodeList %>%
 ...
 print(paste("alpha value", ID))
  ...
  param_check <- 1:nrow(params) %>%
   map_df(~{
          ...
          OriginBlock <- Find_network_balance(List_of_BiConComps[[11]], force = "Net_Generation",
                                        tstep = params$tstep[1], tol = 1e-10, distance = "distance",
                                        maxIter = params$Iterations[1], mass = 5000)$NodeList %>%
...
alpha_Inf_15k2 <- final_z %>%
    Calc_weighted_angle(IEEE_118, .)  

alpha_Inf_15k3 <- final_z2 %>%
    Calc_weighted_angle(IEEE_118, .)  
...
test %>%
  ggplot(aes(x = t, y = z, colour = type)) + geom_line() 
