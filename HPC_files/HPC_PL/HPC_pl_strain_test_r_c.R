...
parameter_df_temp <-  generate_pl_parameters(load_file) %>% #arrange(compute_group) %>% #temporary to get timings
  filter(carrying_capacity == 1| carrying_capacity == Inf, #this variable is inserted into the file
         simulation_id ==1) #only 1 sim from each set needs the strain calculated as they are all the same.

parameter_df_temp <-expand.grid(r = c(1,10,100, 1000, 10000), 
                                c = c(1,10,100, 1000, 10000),
                                tstep = seq(0.0,0.05, 0.005)[-1],
                                coef_drag = seq(0, 5, 5/10)[-1]
                                ) %>% as_tibble() %>%
...
1:nrow(parameter_df_temp) %>%
  walk(~{
    ...
    print(paste("Time for iteration to complete",iteration_time_stop-iteration_time_start))
