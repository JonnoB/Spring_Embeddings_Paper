...
This isn't very interesting at the moment eventhough it was the inspiration for all the work.
Will leave for now but may delete later
...
test2 <- AttackRoundData %>%
  filter(simulationID==5689)
ggplot(test2, aes(x = NodesAttacked, y = PowerGen)) + geom_line()
...
##functions
Add these to the useful autoload folder
Possibly make folder a package
```{r}
#The brackets just ensure all the code is run at once
MakeNodeCommunity <-function(JointFailureProbg, large = 20, medium = 10){
...
NodeCommunity %>%
  left_join(CommunityRank, by = "membership") %>%
  mutate( Community = case_when(
      size>=large ~ as.character(rank),
      size>=medium ~ "medium",
      TRUE ~ "small"
    ),
     Community2 = case_when(
      size<large & size>=medium~ as.character(rank),
      size>=large ~ "Large",
      TRUE ~ "small"
    ))
}
...
IsSameCommunity2 <-function(Community1, Community2){
  ...
  Community1 <- Community1 %>% full_join(Community2 %>% select(Node), by = "Node") %>% arrange(Node)
  Community2 <- Community2 %>% full_join(Community1 %>% select(Node), by = "Node") %>% arrange(Node)
...
ElecDistCommunity <- ImpPTDF(g, SlackRef)$Imp %>%
  ElectricalDistance() %>%
  as.tibble %>%
  mutate(Node1 = names(.)) %>%
...
AdmitTopology <-as_data_frame(g) %>% as_tibble %>%
  mutate(from2 = ifelse(PowerFlow>0, from, to),
         to2 = ifelse(PowerFlow>0, to, from),
         from = from2,
         to = to2,#this is so confusing I am keeping in the columns to make it easier to check back on
...
cluster_walktrap() %>% MakeNodeCommunity(large = 9, medium = 5)
...
#It's a mess
CommSim %>%
  ggplot(aes(x = rank, y = Jaccard, colour = Community)) + geom_point() +
