...
OriginBlock <- Find_network_balance(List_of_BiConComps[[147]], tstep = 0.03, tol = 1e-10, distance = "distance", 
                                      maxIter = 20000, mass = 1)

  final_z <- Create_stabilised_blocks(g2, OriginBlock, 147, tstep = 0.004, tol = 1e-10, 
                                      distance = "distance", maxIter = 40000, mass = 1)
  
  write_rds(final_z, file.path(PLwd, "UK_power_grid_3d.rds"))
  
}

OriginBlock2$results %>%
  ggplot(aes(x = t, y = z)) + geom_line()


test <-OriginBlock2$NodeList
...
z_network_map <-MakeMapDF(g2, read_csv(file.path(basewd, "point.csv")))  %>%
...
# mutate(z = z/max(z)) %>%
 ...
 # geom_line(data = nodes_shortest_path_df, # shortest path no longer necessary as angle is no longer used
  #          aes(group = Link),
   #         colour = "red")+
  facet_grid(~type) +bind_rows(kriged_df, kriged_df_strain) %>%
mutate(type = ifelse("node z"==type, "Height", "Strain")) %>%
  group_by(type) %>%
   mutate(z = percent_rank(z)) %>%
# mutate(z = z/max(z)) %>%
...
scale_colour_viridis_c(option = "plasma") +
  labs(title = "Height and Strain of the UK high-voltage power grid after converting to a system of springs", fill = "Percentile") +
...
line_strain %>%
  ggplot(aes(x = line_load, y = strain)) + geom_point()
