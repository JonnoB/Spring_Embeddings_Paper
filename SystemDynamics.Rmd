...
elec_stacked <- file.path(basewd, "MSOA_demand", "elec_MSOA_dom_stacked.csv") %>% read_csv() %>%
  ...
  mutate(mean_Mw = total.kwh/(365*24*1000))
...
test <- Node_dictionary %>% rename(LSOA11CD = lsoa) %>%
...
test2 <- test %>%
  ...
  mutate(weighted_MW = Pop_perc*mean_Mw)

test_time <- test2 %>%
  group_by(year, Node) %>%
  summarise(total_MW = sum(weighted_MW))

early_nodes <- test_time %>%
  filter(year<2012) %>% pull(Node) %>%  unique


test3 <- test_time %>%
  filter(!(Node %in% early_nodes))


#A test time plot.
#A large number of nodes do not appeara before 2012
test_time %>%
  ggplot(aes(x = year, y = total_MW, group = Node),) + geom_line(alpha = 0.2)

```


#ETYS projections
This data can be used to project node demand into the future and so predict future system strain.
```{r}
