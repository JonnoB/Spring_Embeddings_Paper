...
IEEE_theta_by_alpha_ec <- c("constant_ec_from_alpha_norm_50", "constant_ec_from_alpha_norm") %>%
  map_df(~{
    
    test <- list.dirs(file.path(IEEE_Project_folder, .x), recursive = FALSE) %>%
  map_df(~{ #map loop 2
    Out <- list.files(.x, full.names = TRUE) %>%
      map_df(~{ #map loop 1 inner map
        read_rds(.x) %>%
    Calc_line_strain(IEEE_118_dist , ., distance = "distance", capacity = "Link.Limit", flow = "PowerFlow") %>%
      summarise(strain = mean(strain)) %>%
    #angle_from_solved_heights(.) %>%
  ...
  }) %>% mutate(scramble_fract = .x)
    
  }) %>%
  mutate(scramble_fract = case_when(
    sub(".*_", "",scramble_fract) == "50" ~"50",
    TRUE ~"100"
  ))
...
IEEE_theta_by_alpha_ec <- list.dirs(file.path(IEEE_Project_folder,"constant_ec_from_alpha_norm"), recursive = FALSE) %>%
  map_df(~{
    Out <- list.files(.x, full.names = TRUE) %>%
      map_df(~{
        read_rds(.x) %>%
...
theta_crit_thresh <- AttackRoundData %>%
  ...
  mutate(alpha = 1/alpha,
         orig_strain = strain,
     strain =   relative_position(strain, IEEE_theta_by_alpha$strain) 
     ) %>%
...
#reshape so that the alpha and strain values can be plotted
 ...
 scrambled_ec_points <- theta_crit_thresh_ec_2 %>%
               select(alpha, NodesAttacked, strain, ec, v, scramble_fract) %>%
               gather(key = type, value = value, -NodesAttacked, -ec, -v, -scramble_fract) %>%
  mutate(ec = factor(ec)#,
...
theta_crit_thresh_ec_2  %>%
  mutate(value = NodesAttacked) %>%
  filter(as.character(ec)%>% as.numeric()<30) %>%
  ggplot(aes(y = alpha, x = orig_strain, colour = factor(ec), shape  = scramble_fract)) + 
  geom_point() +
...
PropLoad_line  %>%
 ...
 # geom_line(data = scrambled_ec_points, aes(group = factor(v), colour = ec)) +
  ...
  filter(as.character(ec)%>% as.numeric()<30) %>% filter(bounded),# %>%  filter(ec !=100),
             aes(colour = ec, shape = scramble_fract)) +
