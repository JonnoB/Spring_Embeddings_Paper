...
PropLoad_line <- theta_crit_thresh %>%
  ...
  mutate(type = type %>%factor(., labels = c("1/alpha[sys]", "epsilon~as~fraction~of~max~range")))
...
scrambled_ec_points <- theta_crit_thresh_ec_2 %>%
  ...
  mutate(ec = factor(ec),
         type = type %>%factor(., labels = c("1/alpha[sys]", "epsilon~as~fraction~of~max~range"))
...
#plot the data to show that the strain value has a closer relationship to 
PropLoad_line %>%
  ggplot(aes(y = 1/value, x = NodesAttacked)) + 
  geom_line() +
 # geom_line(data = scrambled_ec_points, aes(group = factor(v), colour = ec)) +
  geom_point(data = scrambled_ec_points,# %>%  filter(ec !=100),
             aes(colour = ec)) +
