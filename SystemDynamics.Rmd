...
nodes_pre_names <- tibble(name = c("Generation", "Transfer", "Demand 1", "Demand 2"), Generation = c(20, 0,0,0), Demand = c(0,0,10,10), force = c(20, 0, -10, -10),
                    norm_force = c(1, 0, -0.5, -0.5))
...
simple_converge <- Find_network_balance_expanded(g_4_3, force = "force", flow ="flow", 
                                                 #capacity = "capacity", 
                                                 tstep = 0.1,
                                                 distance = "distance", 
                                                 mass = 100, 
...
test <- Find_network_balance_expanded(g_4_3, force = "force", 
                                      flow ="flow", 
                                     # capacity = "capacity", 
                                      tstep = 0.1,
                                      distance = "distance", 
...
test2_raw <- Find_network_balance(g_4_3, force = "force", 
                              flow ="flow", 
                              capacity = "capacity", 
                              tstep = 0.1,
                              distance = "distance", 
...
test2 <- test2_raw$NodeList %>%
  mutate(type = "base2")


test_post <- Find_network_balance_expanded(g_post_names, 
                                           force = "force", 
                                           flow ="flow", 
#                                           capacity = "capacity", 
                                           tstep = 0.1,
...
bind_rows(test2, test_post) %>%
  group_by(type) %>%
  filter(t==max(t)) %>%
    ungroup %>%
  ggplot(aes(y = z, x = t, colour = node)) + geom_point() +
  facet_wrap(~type)


bind_rows(test, test_post) %>%
  ggplot(aes(y = z, x = t, colour = node)) + geom_line() +
  facet_wrap(~type)

final <- bind_rows(test2, test_post) %>%
  group_by(type) %>%
  filter(t==max(t)) %>%
    ungroup
...
c(1, Inf, mean_alpha) %>% walk(~{
  ...
  filename <- file.path(PLwd, paste0("base_load_", alpha, ".rds"))
...
sim_names %>%
  walk(~{
    ...
    if(!file.exists(strain_file_path)){
      ...
      giant_componant <-List_of_BiConComps %>% map_dbl(~vcount(.x))
      
      #use the largest block to set the simulation parameters k and m.
      #k needs to be sufficiently stretch to allow enough topology variation. otherwise all that happens is a surface angled in the direct of net power flow. Which is interesting but not that interesting
      OriginBlock <- Find_network_balance(List_of_BiConComps[[which.max(giant_componant)]], 
                                          tstep = 0.05, tol = 1e-10, distance = "distance", 
                                          maxIter = 20000, mass = 1)
      
      final_z <- Create_stabilised_blocks(g2, OriginBlock, which.max(giant_componant), tstep = 0.01, tol = 1e-10, 
                                          distance = "distance", maxIter = 20000, mass = 1)
...
strain_profile_type <- sim_names %>%
  map_df(~{
    ...
    read_rds(strain_file_path) %>%
      Calc_line_strain(set.edge.attribute(get(.x), "distance", value = 1), ., distance) %>%
      mutate(sim = .x,
             edges = ecount(get(.x)))
...
kriged_sim <- sim_names[-2] %>%
  map_df(~{
    ...
    Out2 <- kriged_height_strain_data <- krige_height_strain_maps(g = set.edge.attribute(get(.x), "distance", value = 1), 
                                                      ...
                                                      coords = MakeMapDF(get(.x), read_csv(file.path(basewd, "point.csv")))  %>%
                                                        filter(PositionType=="Geo Space"), distance,
                                                      spatial_df = spdf) %>%
      mutate(simulation = .x)
...
AttackRoundData %>%
  ...
  ggplot(aes(y = perc_edges, x = mean_strain, colour = sim)) + geom_point()
